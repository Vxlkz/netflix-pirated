...
class Video {
    ...
    public function getSeasonAndEpisode() {
        ...
        $season = $this->getSeasonNumber(); // Fixed variable name and colon issue
        $episode = $this->getEpisodeNumber();
    
        return "Season " . $season . ", Episode " . $episode; // Fixed concatenation
    }
    ...
    public function isInProgress($username) {
        ...
        $query->bindValue(":username", $username); // Corrected binding for username
        $query->execute();
    
        return $query->rowCount() != 0; // Fixed case for rowCount
    }
